...
//            p($imageModel->tableData($filter));
            return $imageModel->tableData($filter);
        }
        return $this->fetch('index');
    }

    // 阿里云存储    save(&$file, $replace = true)
    function aliOss()
    {
        // 获取表单上传文件 例如上传了001.jpg
        $config = [
            'accessKeyId'     => c('alioss_key'), //阿里云accesskeyid，用户AccessKey控制台地址：https://usercenter.console.aliyun.com/#/manage/ak
            'accessKeySecret' => c('alioss_secret'), //访问密钥
            'endpoint'        => c('alioss_endpoint'), //访问域名
            'bucket'          => 'theliuqi', //空间名称 c('alioss_bucket')
        ];
        $aliyun = new \org\upload\driver\Aliyun($config);
        $result = $aliyun->save($_FILES['file']);

        if(isset($result['info']['url'])){
            // 存进数据库
            $data['id'] = md5($_FILES['file']['name'].time());
    ...
    public function Admin()
    {
        ...
        $CONFIG = json_decode(preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($config)), true);p($CONFIG);
        $action = input('action');
        switch ($action) {
            case 'config':
